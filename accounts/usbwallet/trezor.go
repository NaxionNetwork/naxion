...
// or whether the Naxion app was not started on it.
...
// Naxion address located on that path.
func (w *trezorDriver) trezorDerive(derivationPath []uint32) (common.Address, error) {
	address := new(trezor.NaxionAddress)
	if _, err := w.trezorExchange(&trezor.NaxionGetAddress{AddressN: derivationPath}, address); err != nil {
...
func (w *trezorDriver) trezorSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	response := new(trezor.NaxionTxRequest)
	...
	for response.DataLength != nil && int(*response.DataLength) <= len(data) {
		...
		if _, err := w.trezorExchange(&trezor.NaxionTxAck{DataChunk: chunk}, response); err != nil {
			return common.Address{}, nil, err
		}
	}
