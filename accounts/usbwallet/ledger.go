...
const (
	ledgerOpRetrieveAddress  ledgerOpcode = 0x02 // Returns the public key and Naxion address for a given BIP 32 path
	ledgerOpSignTransaction  ledgerOpcode = 0x04 // Signs an Naxion transaction after having the user validate the parameters
...
func (w *ledgerDriver) Status() (string, error) {
	...
	if w.browser {
		return "Naxion app in browser mode", w.failure
	}
	if w.offline() {
		return "Naxion app offline", w.failure
	}
	return fmt.Sprintf("Naxion app v%d.%d.%d online", w.version[0], w.version[1], w.version[2]), w.failure
}

// offline returns whether the wallet and the Naxion app is offline or not.
//
...
func (w *ledgerDriver) Open(device io.ReadWriter, passphrase string) error {
	...
	if err != nil {
		// Naxion app is not running or in browser mode, nothing more to do, return
		if err == errLedgerReplyInvalidHeader {
			w.browser = true
		}
		return nil
	}
	// Try to resolve the Naxion app's version, will fail prior to v1.0.2
...
// and returning the Naxion address located on that derivation path.
...
//
// Note, if the version of the Naxion application running on the Ledger wallet is
// too old to sign EIP-155 transactions, but such is requested nonetheless, an error
// will be returned opposed to silently signing in Homestead mode.
func (w *ledgerDriver) SignTx(path accounts.DerivationPath, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	// If the Naxion app doesn't run, abort
	if w.offline() {
...
// ledgerVersion retrieves the current version of the Naxion wallet app running
...
// ledgerDerive retrieves the currently active Naxion address from a Ledger
...
//   Naxion address length | 1 byte
//   Naxion address        | 40 bytes hex ascii
...
func (w *ledgerDriver) ledgerDerive(derivationPath []uint32) (common.Address, error) {
	...
	// Extract the Naxion hex address string
	if len(reply) < 1 || len(reply) < 1+int(reply[0]) {
		return common.Address{}, errors.New("reply lacks address entry")
	}
	hexstr := reply[1 : 1+int(reply[0])]

	// Decode the hex sting into an Naxion address and return
...
func (w *ledgerDriver) ledgerSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	// Extract the Naxion signature and do a sanity validation
	if len(reply) != 65 {
