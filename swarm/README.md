...
To run Swarm you need an Naxion account. You can create a new account by running the following command:
...
Once you have specified the password, the output will be the Naxion address representing that account. For example:
...
### Naxion Name Service resolution

The Naxion Name Service is the Naxion equivalent of DNS in the classic web. In order to use ENS to resolve names to Swarm content hashes (e.g. `bzz://theswarm.eth`), `swarm` has to connect to a `geth` instance, which is synced with the Naxion mainnet. This is done using the `--ens-api` flag.
...
Swarm offers a local HTTP proxy API that Dapps can use to interact with Swarm. The Naxion Foundation is hosting a public gateway, which allows free access so that people can try Swarm without running their own node.
