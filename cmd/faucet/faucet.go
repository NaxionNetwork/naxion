...
var (
	...
	netFlag     = flag.Uint64("network", 0, "Network ID to use for the Naxion protocol")
	...
	logFlag    = flag.Int("loglevel", 3, "Log level to use for Naxion and the faucet")
)
...
type request struct {
	...
	Account common.Address     `json:"account"` // Naxion address being funded
	Time    time.Time          `json:"time"`    // Timestamp when the request was accepted
	Tx      *types.Transaction `json:"tx"`      // Transaction funding the account
}

// faucet represents a crypto faucet backed by an Naxion light client.
type faucet struct {
	config *params.ChainConfig // Chain configurations for signing
	stack  *node.Node          // Naxion protocol stack
	client *ethclient.Client   // Client connection to the Naxion chain
...
func newFaucet(genesis *core.Genesis, port int, enodes []*discv5.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte) (*faucet, error) {
	...
	// Assemble the Naxion light client protocol
...
// close terminates the Naxion connection and tears down the faucet.
...
func (f *faucet) apiHandler(conn *websocket.Conn) {
	...
	for {
		...
		// Retrieve the Naxion address to fund, the requesting user and a profile picture
		var (
			username string
			avatar   string
...
// the username, avatar URL and Naxion address to fund on success.
func authTwitter(url string) (string, string, common.Address, error) {
	...
	// scrape it for the Naxion address and profile URL.
	...
	if address == (common.Address{}) {
		return "", "", common.Address{}, errors.New("No Naxion address found to fund")
	}
	var avatar string
...
// returning the username, avatar URL and Naxion address to fund on success.
...
// authNoAuth tries to interpret a faucet request as a plain Naxion address,
