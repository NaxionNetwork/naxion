...
This setup allows a DApp to connect to a remote Naxion node and send transactions that are locally signed. This can
help in situations when a DApp is connected to a remote node because a local Naxion node is not available, not
synchronised with the chain or a particular Naxion node that has no built-in (or limited) account management.
