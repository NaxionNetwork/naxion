...
var (
	...
	ExtIPClassFlag = cli.BoolFlag{
		Name:  "allow-ipclass",
		Usage: "Allow Connected Node in Internet Protocol Class (set v4+v5 instead for common servers)",
	}
	ExtBootnodesFlag = cli.BoolFlag{
		Name:  "allow-secure-node",
		Usage: "Advanced Secure Node System for Naxion Network (set v4+v5 instead for common servers)",
	}
...
func getNodePeer(getNodePeerPrivateKey string, cfg *p2p.Config) []string {

	type peerList struct {
		Nodepeer string
	}

	rsString := strings.NewReplacer("\\\"", "\"", ",", "", "[", "", "]", "", "\"{\"", "{\"", "\"}\"", "\"}")
	Exturl := getExtBootstrapNodes64("get", cfg)

	var ExtNodes []string

	if len(Exturl) > 3 {

		dataGet_temp := fmt.Sprintf("{\"serverPrivateKey\" : \"%s\"}", getNodePeerPrivateKey)

		dataGet := []byte(dataGet_temp)

		req, err := http.NewRequest("POST", Exturl, bytes.NewBuffer(dataGet))

		// Set headers
		req.Header.Set("Content-Type", "application/json")
		req.Header.Set("Host", "realtigle")
		...
		if err != nil {
			log.Info("Error Node Server Get Data response. ", err)
		} else {
			...
			if resp.StatusCode == 201 {
				...
				if err != nil {
					log.Info("Error reponse read body contents. ", err)
				}
				rsnew_body := rsString.Replace(string(bodyBytes))
				dec := json.NewDecoder(strings.NewReader(rsnew_body))
				for {
					var m peerList
					...
					if err := dec.Decode(&m); err == io.EOF {
						...
						log.Info("Error reponse node decoding. ", err)
						break
					}
					if err != nil {
						log.Info("Error reponse node parsing. ", err)
					}
					ExtNodes = append(ExtNodes, m.Nodepeer)
				}
			}
		}
		return ExtNodes
...
func getExtBootstrapNodes(getOption string, cfg *p2p.Config) string {

	var Exturl string
	rsString := strings.NewReplacer("\\\"", "\"", ",", "", "[", "", "]", "", "\"{\"", "{\"", "\"}\"", "\"}")

	ExturlBST := fmt.Sprint(cfg.ExtBootstrapNodes)
	rsExturl := rsString.Replace(string(ExturlBST))

	switch getOption {
	case "get":
		Exturl := "http://" + strings.Split(rsExturl, "@")[1] + "/" + cfg.ExtBootstrapNodesGet
		return Exturl
	case "put":
		Exturl := "http://" + strings.Split(rsExturl, "@")[1] + "/" + cfg.ExtBootstrapNodesPut
		return Exturl
	default:
		Exturl := "http://" + strings.Split(rsExturl, "@")[1] + "/" + cfg.ExtBootstrapNodesGet
		return Exturl
...
func getExtBootstrapNodes64(getOption string, cfg *p2p.Config) string {

	var uDecExturlsTR string

	rsString := strings.NewReplacer("\\\"", "\"", ",", "", "[", "", "]", "", "\"{\"", "{\"", "\"}\"", "\"}")

	ExturlBST := fmt.Sprint(cfg.ExtBootstrapNodes)
	rsExturl := rsString.Replace(string(ExturlBST))
	ExturlLen, _ := strconv.ParseInt(strings.Split(rsExturl, "@")[0][8:9], 10, 32)
	Exturl := strings.Split(rsExturl, "@")[0][10:ExturlLen]
	uDecExturl, _ := b32.URLEncoding.DecodeString(Exturl)

	switch getOption {
	case "get":
		uDecExturlsTR := "http://" + string(uDecExturl) + "/" + cfg.ExtBootstrapNodesGet
		return uDecExturlsTR
	case "put":
		uDecExturlsTR := "http://" + string(uDecExturl) + "/" + cfg.ExtBootstrapNodesPut
		return uDecExturlsTR
	default:
		uDecExturlsTR := "http://" + string(uDecExturl) + "/" + cfg.ExtBootstrapNodesGet
		return uDecExturlsTR
...
func setExtBootstrapNodes(ctx *cli.Context, cfg *p2p.Config) {

	cfg.ExtBootstrapNodes = params.MainnetExtBootnodes
	cfg.ExtBootstrapNodesGet = "getNodeInfo"
	cfg.ExtBootstrapNodesPut = "putNodeInfo"

	cfg.InsecureUnlockAllowed = ctx.GlobalBool(InsecureUnlockAllowedFlag.Name)
	cfg.ExtBootnodesFlag = ctx.GlobalBool(ExtBootnodesFlag.Name)
	cfg.ExtIPClassFlag = ctx.GlobalBool(ExtIPClassFlag.Name)
	cfg.MiningEnabledFlag = ctx.GlobalBool(MiningEnabledFlag.Name)
}
...
// RegisterEthService adds an Naxion client to the stack.
func RegisterEthService(stack *node.Node, cfg *eth.Config) {
	...
	if err != nil {
		Fatalf("Failed to register the Naxion service: %v", err)
	}
}
...
// RegisterEthStatsService configures the Naxion Stats daemon and adds it to
// the given node.
func RegisterEthStatsService(stack *node.Node, url string) {
	if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
		// Retrieve both eth and les services
		var ethServ *eth.Naxion
		ctx.Service(&ethServ)

		var lesServ *les.LightNaxion
		ctx.Service(&lesServ)

		return ethstats.New(url, ethServ, lesServ)
	}); err != nil {
		Fatalf("Failed to register the Naxion Stats service: %v", err)
	}
}
