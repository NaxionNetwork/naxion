...
// nodeDockerfile is the Dockerfile required to run an Naxion node.
var nodeDockerfile = `
...
// an Naxion node (bootnode or miner for now).
var nodeComposefile = `
version: '2'
services:
  {{.Type}}:
...
// deployNode deploys a new Naxion node container to a remote machine via SSH,
