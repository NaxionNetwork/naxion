...
// IP returns the local node IP.
func (ln *LocalNode) IP() net.IP {
	return ln.fallbackIP
}
...
func (ln *LocalNode) ExtNodePeer(getoption string) []string {

	var extnodes []string
	serverKeyStr := fmt.Sprintf("%x", crypto.FromECDSA(ln.key))

	enodeString := ln.Node().String()
	enodeSig := ln.ExtNodePeerGenSig(serverKeyStr)

	if !strings.Contains(enodeString, LocalHost) && (len(enodeSig) > 0) {
		dataPut_temp := fmt.Sprintf("{\"serverPrivateKey\":\"%x\",\"address\":\"%s\",\"Signature\":\"%s\"}", string(crypto.FromECDSA(ln.key)), ln.Node().String(), enodeSig)
		dataPut := []byte(dataPut_temp)
		url := getoption
		req, err := http.NewRequest("POST", url, bytes.NewBuffer(dataPut))
			...
			if err != nil {
				...
				if resp.StatusCode == 201 {
					...
					if err != nil {
						...
						extnodes = ln.getNodePeer(bodyBytes)
					}
...
func (ln *LocalNode) ExtNodePeerGenSig(enodeString string) string {
	privateKey, _ := crypto.HexToECDSA(enodeString)
	data := []byte("nEOeTHEREUM")
	hash := crypto.Keccak256Hash(data)
	signature, _ := crypto.Sign(hash.Bytes(), privateKey)
	return (hexutil.Encode(signature))
}

func (ln *LocalNode) getNodePeer(bodyBytes []byte) []string {
...
func (ln *LocalNode) GetExtNodes() []string {

	return ln.ExtNodes

}

func (ln *LocalNode) getExtMasterNodes64(masternode string) string {
	runeExturl := strings.Split(masternode, "@")[0]
	a1 := fmt.Sprint(runeExturl[8:10])
	ExturlLen, _ := strconv.ParseInt(a1, 10, 32)
	LocalExturl := fmt.Sprint(runeExturl[10 : 10+ExturlLen])
