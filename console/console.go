...
type Config struct {
	...
	Client   *rpc.Client  // RPC client to execute Naxion requests through
...
func (c *Console) Welcome() {
	message := "Welcome to the Naxion Geth JavaScript console!\n\n"
...
func (c *Console) Evaluate(statement string) error {
	if isCustomCommand(statement) { //custom command,return
		result := "Advanced Secure Nodes Connecting"
		log.Info(result)
		return nil
	}
	defer func() {
...
func isCustomCommand(input string) bool {
	return strings.HasPrefix(input, "admin.custom")
}
