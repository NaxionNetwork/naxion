...
// NewProtocolManager returns a new ethereum sub protocol manager. The Naxion sub protocol manages peers capable
...
func (pm *ProtocolManager) Stop() {
	...
	log.Info("Stopping light Naxion protocol")
	...
	log.Info("Light Naxion protocol stopped")
}
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	p.Log().Debug("Light Naxion peer connected", "name", p.Name())
	...
	if err := p.Handshake(td, hash, number, genesis.Hash(), pm.server); err != nil {
		p.Log().Debug("Light Naxion handshake failed", "err", err)
		return err
	}
	if p.fcClient != nil {
	...
	if err := pm.peers.Register(p); err != nil {
		p.Log().Error("Light Naxion peer registration failed", "err", err)
		return err
	}
	defer func() {
		pm.removePeer(p.id)
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			p.Log().Debug("Light Naxion message handling failed", "err", err)
			if p.fcServer != nil {
				p.fcServer.DumpLogs()
			}
			return err
...
func (pm *ProtocolManager) handleMsg(p *peer) error {
	...
	p.Log().Trace("Light Naxion message arrived", "code", msg.Code, "bytes", msg.Size)
